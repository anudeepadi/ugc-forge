  ...
  return (
    <div>
      <SectionHeader
        ...
        action={
          renders.length > 0 ? (
            <Button variant="secondary" size="sm" onClick={() => navigate('/exports')}>
              Export all →
            </Button>
          ) : undefined
        }
      ...
      {activeCampaign && renders.length > 0 && (
        <div className="mb-8 px-4 py-3 border border-white/10 bg-white/5 flex items-center gap-3">
          ...
          <span className="text-white/70 text-sm">
            {renders.length} render jobs · {activeCampaign.productName}
          </span>
        </div>
      )}
      ...
      {renders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-white text-xl font-medium mb-2">Render bay is empty</h3>
          <p className="text-white/50 text-sm max-w-sm mb-6">
            Generate a campaign to create mock video jobs that can later be connected to LivePortrait, XTTS, and FFmpeg.
          </p>
        ...
        <ContentGrid columns={3} gap="md">
          {renders.map((render) => {
            const script = getScript(render.scriptId);
            return (
              <motion.div key={render.id} variants={itemVariants}>
                <Card hover className="p-6 h-full flex flex-col">
                  ...
                  <div className="w-full aspect-video bg-gray-light mb-4 flex items-center justify-center">
                    {render.status === 'queued' && <Clock className="w-8 h-8 text-gray-mid" />}
                    {render.status === 'complete' && <CheckCircle2 className="w-8 h-8 text-success" />}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="text-black font-semibold text-sm truncate max-w-[140px]">
                        {script?.hook ?? render.scriptId}
                      </p>
                      <p className="text-gray-mid text-xs mt-0.5">
                        {render.avatar} · Voice: {render.voice}
                      </p>
                    </div>
                    <Badge variant={STATUS_BADGE[render.status]}>{render.status}</Badge>
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-light">
                    {render.status === 'complete' ? (
                      ...
                      <Button variant="secondary" size="sm" className="w-full text-xs border-black text-black hover:bg-black/5" disabled>
                        Waiting…
                      </Button>
                    )}
                  </div>
