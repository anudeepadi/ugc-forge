      ...
      {scripts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-white/50 text-sm max-w-sm mb-6">
            Generate a campaign to get viral score estimates for each script
            variant.
          </p>
        ...
        <>
          ...
          <Card className="p-6 mb-8">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-section-header text-lg text-black">
                Top performers
              </h3>
            </div>
            <div className="space-y-3">
              {sorted.slice(0, 5).map((s, i) => (
                <div
                  key={s.id}
                  className="flex items-center gap-4 py-2 border-b border-gray-light last:border-0"
                >
                  <span className="text-display text-2xl text-gray-mid w-6">
                    {i + 1}
                  </span>
            ...
            {sorted.map((script) => (
              <motion.div key={script.id} variants={itemVariants}>
                <Card hover className="p-6 flex flex-col h-full">
                  ...
                  <p className="text-black font-semibold text-sm leading-snug mb-2">
                    {script.hook}
                  </p>
                  <p className="text-gray-mid text-xs line-clamp-2 flex-1">
                    {script.body}
                  </p>
